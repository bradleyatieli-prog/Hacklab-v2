...
if [ "$HAVE_GUM" = 1 ]; then
    ...
    # The box above already hides "2) root" entirely when not rooted —
    # match that here too. A hardcoded "[1-3]" prompt next to a box
    # that only ever showed "1)" and "3)" is exactly the mismatch that
    # looks like a rendering bug rather than an intentional, root-gated
    # option being hidden.
    if [ "$ROOTED" = 1 ]; then
        printf "  ${C_WHITE}${BOLD}Choose [1-3]:${NC} "
    else
        printf "  ${C_WHITE}${BOLD}Choose [1 or 3]:${NC} "
    fi
    read -r path_choice
fi
