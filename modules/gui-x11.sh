...
#
# Only the apt family gets a `|| apt-get --fix-broken install` retry
# baked into the command. That's deliberate, not an oversight: dpkg's
# unpack-then-configure-in-topological-order two-phase install is
# specifically what can leave a big interdependent metapackage (lxqt's
# lxqt-*/kf6-*/qt6-* web is a real example of this) "unpacked but not
# configured" after one pass — pacman/dnf/zypper/apk all resolve the
# full dependency graph before installing anything, so they don't hit
# this particular failure mode the same way.
de_install_cmd() {
    ...
    case "$family:$style" in
        apt:xfce)     echo "apt-get update && (apt-get install -y xfce4 dbus-x11 || apt-get --fix-broken install -y)" ;;
        apt:lxqt)     echo "apt-get update && (apt-get install -y lxqt dbus-x11 || apt-get --fix-broken install -y)" ;;
        apt:i3)       echo "apt-get update && (apt-get install -y i3 dmenu dbus-x11 xterm || apt-get --fix-broken install -y)" ;;
...
install_distro() {
    ...
    if [ -z "$de_cmd" ]; then
        ...
        # This used to only warn() on failure and fall straight through
        # to writing the state file and printing "ready" anyway — the
        # exact same false-success bug just found and fixed in
        # core/menu.sh's install loop, just unfixed here. apt-based
        # chroots (kali/ubuntu/debian) run the identical dpkg/apt
        # machinery that just failed on lxqt for the light path
        # (de_install_cmd already retries with `apt-get --fix-broken
        # install` for exactly that reason) — but if it genuinely still
        # fails after that, this needs to actually report it as a
        # failure, not silently mark the module ready anyway.
        if ! proot-distro login "$name" -- bash -c "$de_cmd"; then
            err "desktop package install inside $name failed — package names can drift between releases."
            err "Check manually with: proot-distro login $name"
            return 1
        fi
    fi
...
case "$1" in
  ...
  next_steps)
    if [ ! -f "$STATE_FILE" ]; then
        echo "Select gui-x11 again to finish choosing a path."
        exit 0
    fi
    state="$(cat "$STATE_FILE")"
    case "$state" in
        light:*)
            style="${state#light:}"
            cat << EOF
1) Open the Termux-X11 app (install it from F-Droid/Play if you haven't already).
2) Come back here and select gui-x11 again — it launches straight into your $style desktop.
EOF
            ;;
        distro:*)
            rest="${state#distro:}"
            ...
            cat << EOF
1) Open the Termux-X11 app (install it from F-Droid/Play if you haven't already).
2) Select gui-x11 again here — it logs into the $name chroot and launches $style inside it.
3) Manual shell access any time: proot-distro login $name
EOF
            ;;
        *)
            echo "Select gui-x11 again to launch it."
            ;;
    esac
    ;;
  *)
    echo "Usage: $0 {install|switch|run|next_steps}"
    exit 1
    ;;
esac
