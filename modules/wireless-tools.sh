...
case "$1" in
  ...
  next_steps)
    cat << EOF
aircrack-ng is ready. Select wireless-tools again any time to drop into a shell with it on PATH.
Reminder: monitor mode is usually blocked by Android Wi-Fi firmware — packet
injection / deauth will likely not work here regardless of root.
EOF
    ;;
  run)
