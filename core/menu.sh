...
confirm_action() {
    ...
    if verify_cmd gum --version; then
        gum confirm "$prompt"
    elif verify_cmd dialog --version; then
        # --yesno's own exit code is exactly true/false, so just
        # propagate it — 0 only on an explicit "Yes", same as gum
        # confirm and the plain read tier below.
        dialog --clear --yesno "$prompt" 0 0
        local rc=$?
        tty_clear
        return $rc
    else
...
pause_prompt() {
    if verify_cmd gum --version; then
        ...
        dialog --msgbox "press enter to return to menu" 0 0
        tty_clear
    else
