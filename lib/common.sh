...
# untouched. Heights/widths are 0 0 (dialog auto-sizes to content and
# to the real terminal) rather than a hardcoded width, for the same
# reason install.sh's BOX_W is now auto-detected: phones in portrait
# Termux are often well under 60 columns.
dialog_menu() {
    ...
    local tag rc
    exec 3>&1
    tag="$(dialog --clear --menu "$header" 0 0 "$rows" "${args[@]}" 2>&1 1>&3)"
    rc=$?
    exec 3>&-
    tty_clear
