...
install_pkg_quiet() {
    ...
    #
    # PIPESTATUS[1], not tee's own exit code: modules/gui-x11.sh chains
    # `install_pkg_quiet i3 && install_pkg_quiet dmenu`, so this
    # function's own exit code has to keep reflecting whether the
    # install actually succeeded, not whether `tee` could write a file.
